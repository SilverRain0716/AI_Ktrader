...
def _cand(code, *, f_days=5, i_days=5, fnet=100.0, inet=100.0, vr=1.2, cap=5000.0):
    return U.Candidate(
        code=code,
        name=code,
        market="KOSPI",
        sector="반도체",
        indicators={"volume_ratio": vr, "market_cap_eok_krw": cap},
        flows={
            "foreign_net_days": f_days,
            "inst_net_days": i_days,
            "foreign_net_5d_eok_krw": fnet,
            "inst_net_5d_eok_krw": inet,
        },
    )


def _flow(*cands):
    return [c for c, _r in U._flow_channel({c.code: c for c in cands}, 20)]


def test_한쪽만_사면_뽑지_않는다():
    """순매수 **합계**가 양수여도 한쪽이 다른 쪽을 상쇄한 것일 수 있다 —
    실리콘투(외 +2,078억 / 기 -1,814억)가 그랬다. 그 사례를 근거 문장에만 적고
    필터에는 안 넣었던 것이 이 채널의 원래 결함이다 (ADR 0010).
    """
    assert _flow(_cand("A", fnet=2078.0, inet=-1814.0)) == []
    assert _flow(_cand("B", f_days=5, i_days=0)) == []
    assert _flow(_cand("C")) == ["C"]


def test_거래대금이_식으면_뽑지_않는다():
    """프롬프트가 이미 *"거래대금이 식은 채로 이어지는 매집은 관심이 빠지는 중일 수
    있다"* 고 말하는데 채널이 그 종목을 뽑고 있었다. 실측에서 그 구간이 세 구간 모두
    음수였다 (2026-09-08).
    """
    assert _flow(_cand("A", vr=0.6)) == []
    assert _flow(_cand("B", vr=1.0)) == ["B"]


def test_거래대금을_모르면_통과시키지_않는다():
    """**모르면 통과**로 두면 검사 없이 들어온다 — 이 저장소가 반복해 당한 방식이다."""
    c = _cand("A")
    c.indicators.pop("volume_ratio")
    assert _flow(c) == []


def test_시총_대비_강도로_줄을_세운다():
    """같은 금액이면 작은 회사에 더 큰 사건이다."""
    big = _cand("BIG", fnet=100.0, inet=100.0, cap=100000.0)
    small = _cand("SML", fnet=100.0, inet=100.0, cap=1000.0)
    assert _flow(big, small) == ["SML", "BIG"]


def test_정원을_못_채우는_것이_정상이다():
    """조건을 만족하는 종목이 5개면 5개만 올린다. 20개를 채우려고 나쁜 조합을
    끌어오는 것보다 정직하다."""
    pool = {f"C{i}": _cand(f"C{i}") for i in range(3)}
    assert len(U._flow_channel(pool, 20)) == 3
