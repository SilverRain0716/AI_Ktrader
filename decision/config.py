...
MARGIN_MAX_PCT = 40
# 집행 계층이 실제로 낼 수 있는 진입 방식 (ADR 0009 결정 1).
# **`COND` 는 빠져 있다** — 조건을 감시할 실시간 코드(블록 G)가 0줄이다.
# 블록 G 가 생기면 여기에 추가한다. 그때까지 AI 가 COND 를 내면 게이트가 차단한다.
ALLOWED_ENTRY_TYPES = ("MARKET", "LIMIT")
HALT_LOOKBACK_DAYS = 20  # 최근 이 기간에 거래정지 흔적이 있으면 제외

# 3채널 랭킹. 단일 점수로 줄을 세우면 한 가지 성격의 종목만 상위를 채운다.
#
# **momentum 을 20 → 5 로 줄였다** (2026-09-08). 실측 60거래일에서 세 구간 모두
# 모집단보다 나빴고 +20일에서는 표본 678건에 -7.3%p 였다. `flow` 도 일관되게
# -0.8~-1.6%p 지만 momentum 쪽이 훨씬 크다.
#
# **0 으로 두지 않는 이유**: 측정 구간이 하락장 하나뿐이다(모집단 +20일 -4.94%).
# 추세추종이 하락장에서 나쁜 것은 당연할 수 있고, 완전히 빼면 **상승장에서 어떤지
# 영원히 못 잰다.** 줄여서 계속 재는 쪽을 택했다.
# 재현: `python -m scripts.measure_channel_ablation`
CHANNEL_QUOTA = {"briefing": 20, "momentum": 5, "flow": 20}
UNIVERSE_MAX = 60

# 채널별 조건
#
# **3 → 10 으로 늘렸다** (2026-09-08). 성능을 노린 것이 아니라 **표본을 만들려는 것**이다.
# briefing 채널이 모집단보다 나아 보였지만(+20일 +6.4%p) 그 칸의 표본이 **14건·종목 9개**
# 였다 — 브리핑이 최근 것뿐이라 20거래일 이후 데이터가 있는 것이 그것뿐이었다.
# 기간을 늘리면 +5일 표본이 147 → 258 로 늘고, 몇 주 뒤에 +20일 표본이 생긴다.
# **늘렸다고 좋아졌다고 읽으면 안 된다** — 좋은지 나쁜지는 그때 가서 잰다.
BRIEFING_LOOKBACK_DAYS = 10
