...
# **크론이 실제로 부르는 것은 `ops/crontab.example` 이 정한다.** 여기 있는 시각은
# `--auto` 가 "지금 어느 사이클인가"를 판정할 때와, 손으로 부를 때의 기준이다.
#
# 2026-09-08 에 크론을 **premarket·postmarket 둘로 줄였다.** 8거래일 실적:
#
#   premarket  판단 20 · 매수 23 · $4.12  → 매수당 $0.18
#   midday     판단 23 · 매수  7 · $4.48  → 매수당 $0.64 (3.5배)
#   preclose   **한 번도 안 돌았다** — 15:00 결정은 valid_until 15:20 이라
#              사람이 20분 안에 place 를 쳐야 한다. 현실적으로 안 된다
#
# 그리고 이것은 스윙이다 — CLAUDE.md 실측대로 **하루치 움직임은 손절선까지
# 거리의 1/7 이하**이고 진입 시각 최적화는 2차적이다. 장중 급변은 판단을 다시
# 부르는 것이 아니라 `protect`(손절선)와 `invalidation` 감시가 맡는다.
#
# midday·preclose 는 **지웠다가 아니라 크론에서 뺐다.** 손으로 부를 수 있다.
SCHEDULE: dict[str, time] = {
    # 09:00 — 개장 직후다. 08:20 은 넥스트레이드(08:00~)가 이미 도는데도
    # 장중 시세를 못 봐서 전 거래일 종가로만 판단했다.
    "premarket": time(9, 0),
    "midday": time(12, 20),
    "preclose": time(15, 0),
